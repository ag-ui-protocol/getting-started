...
// `subagentId` was dropped from every other event, since a field absent from the
...
describe("Subagent lifecycle events over protobuf", () => {
  ...
  it("should round-trip SUBAGENT_STARTED with only the required fields", () => {
    ...
    expect(decoded.subagentId).toBe("sub-1");
    ...
    expect(decoded.parentSubagentId).toBeUndefined();
  ...
  it("should round-trip SUBAGENT_FINISHED with and without a result", () => {
    ...
    expect(bare.subagentId).toBe("sub-1");
  ...
  it("should encode each lifecycle event to a non-empty buffer", () => {
    ...
    const events = [
      { type: EventType.SUBAGENT_STARTED, subagentId: "s", name: "n" },
      { type: EventType.SUBAGENT_FINISHED, subagentId: "s" },
      { type: EventType.SUBAGENT_ERROR, subagentId: "s", message: "m" },
    ];
...
describe("subagentId attribution over protobuf", () => {
  it("should preserve subagentId on every attributable event already in the schema", () => {
    ...
    for (const base of events) {
      const decoded = decode(encode({ ...base, subagentId: "sub-7" } as never)) as Record<
        string,
        unknown
      >;
      expect(decoded.subagentId, `${base.type} lost subagentId`).toBe("sub-7");
    }
  });

  it("should leave subagentId absent when the producer omits it", () => {
    ...
    for (const base of events) {
      ...
      expect(decoded.subagentId, `${base.type} invented a subagentId`).toBeUndefined();
    }
  });
  ...
  it("documents that chunk events still cannot be encoded at all (pre-existing)", () => {
    ...
    // PNI-277. Both messages did get a subagent_id field for when they are
    ...
    for (const event of [
      { type: EventType.TEXT_MESSAGE_CHUNK, messageId: "m", delta: "x", subagentId: "s" },
      { type: EventType.TOOL_CALL_CHUNK, toolCallId: "t", delta: "y", subagentId: "s" },
    ]) {
  ...
  it("should preserve per-message subagentId through MESSAGES_SNAPSHOT", () => {
    ...
    const event: MessagesSnapshotEvent = {
      ...
      messages: [
        ...
        { id: "m2", role: "assistant", content: "subagent speaking", subagentId: "sub-1" },
        { id: "m3", role: "assistant", content: "other subagent", subagentId: "sub-2" },
      ],
    ...
    expect((byId.m1 as Record<string, unknown>).subagentId).toBeUndefined();
    expect((byId.m2 as Record<string, unknown>).subagentId).toBe("sub-1");
    expect((byId.m3 as Record<string, unknown>).subagentId).toBe("sub-2");
  });
});
...
describe("wire compatibility of the subagent additions", () => {
  it("should decode bytes produced before the subagent fields existed", () => {
    ...
    // acquire a subagentId out of thin air. If someone renumbers an existing
    // field, this is what fails.
    //
    // TEXT_MESSAGE_START { messageId: "m1", role: "assistant" }, no subagentId.
    ...
    expect(decoded.subagentId).toBeUndefined();
  });
