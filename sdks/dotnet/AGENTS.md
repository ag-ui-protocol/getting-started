...
- `src/AGUI.Server/` — Framework-agnostic server-side adapter (no ASP.NET): `RunAgentInputExtensions.ToChatRequestContext`, `ChatRequestContext`, `ChatResponseUpdateAGUIExtensions.AsAGUIEventStreamAsync`, fluent `AGUIStreamOptions`, `AGUIConstants`. Depends on Abstractions + `M.E.AI`; mixed client/server continuation uses FICC invocation context to correlate client results by call ID.
