...
them, off by default — in the LangGraph integration it is `emit_subagent_events`:
...
With it off, no lifecycle events are emitted, no `subagentRunId` is attributed,
and nothing subagent-related reaches `MessagesSnapshot`, so the stream is what it
was before subagent support existed. Turn it on once every consumer is new
enough.
...
- **Protobuf covers a subset of event types.** 19 of the 36 event types have a
  ...
  start/args/end, state, step, and run families. The rest cannot be encoded at
  all — and several of those carry attribution: `ToolCallResult`, the reasoning
  family, and the activity events. The chunk shorthand is in the same position
  despite having message definitions and a `subagent_run_id` field, since it has
  no enum entry. All of this predates subagent support.
