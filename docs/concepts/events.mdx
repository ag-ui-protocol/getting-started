...
| `metadata`  | Optional extra information attached to the event                 |

### Metadata

`metadata` is an optional, open-by-key object for attaching extra information to
an event — token usage, a trace id, a finish reason. It is declared once on the
base event, so every event type carries it.

Consumers merge an event's metadata into the message that event builds, key by
key, with the last write winning. That is what lets a producer send token usage
on the final event of a message rather than having to know it up front.

See [Metadata](/concepts/metadata) for the full rules: the reserved `ag-ui` key,
which events do and do not merge, how tool calls carry their own, and how the
object behaves across transports.

Most events additionally accept an optional `subagentRunId`, identifying the
subagent that produced them. An event without it belongs to the parent agent.
See [Subagents](/concepts/subagents).
...
Signals the end of an agent run. Every run terminates with either `RunFinished`
or `RunError`.

`RunFinished` has an optional `outcome` discriminated union:

- **omitted** — legacy producer that has not yet adopted the interrupt-aware
  lifecycle. Treated as a normal completion.
- `outcome: { type: "success" }` — the run completed normally. The optional
  `result` stays at the root of the event for back-compat.
- `outcome: { type: "interrupt", interrupts: [...] }` — the run paused for human
  input. The non-empty `interrupts` array lives inside the outcome variant. The
  client resumes by starting a new run whose `RunAgentInput` includes a `resume`
  array addressing every open interrupt.

<Note>
  For the full interrupt lifecycle — the `Interrupt` type, contract rules, error
  handling, reason taxonomy, and worked examples — see
