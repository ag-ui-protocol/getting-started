...
<Note>
The agent will NOT emit Agent User Interaction Protocol events for backend tool calls, and as such the frontend will have no visibility into them.

If you need frontend visibility, you can expose frontend tools for human in the
loop flows, or use the `ChatClientAgent` as a base class and take advantage of
deeper hooks into tool preparation and invocation.
