...
const result = EventSchemas.safeParse(incoming);
if (result.success) {
  // result.data is typed as the discriminated AGUIEvent union
} else {
  console.error(result.error);
}
```

zod is an optional peer dependency of `@ag-ui/core/schemas`. Install it explicitly
if you import from this module:

```bash
npm install zod
# or pnpm/yarn — every zod from 3.25.0 onward is supported, including all of zod 4
```

If you'd rather use a different validation library, define your own schemas locally
using your library of choice. The TypeScript types from `@ag-ui/core` are the contract.
