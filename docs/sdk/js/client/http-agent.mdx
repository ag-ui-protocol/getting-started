...
When using `HttpAgent` in a Next.js application with `output: 'standalone'`, the standalone bundler may break streaming `fetch` calls at runtime, causing HTTP requests to silently fail. To fix this, add `@ag-ui/client` and `@ag-ui/core` to `serverExternalPackages` in your `next.config.mjs`:

```javascript
// next.config.mjs
const nextConfig = {
  output: 'standalone',
  serverExternalPackages: ['@ag-ui/client', '@ag-ui/core'],
}
```
